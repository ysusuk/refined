...
private[refined] trait StringValidate {
  ...
  implicit def endsWithValidate[S <: String](implicit ws: ValueOf[S]): Validate.Plain[String, EndsWith[S]] =
    Validate.fromPredicate(_.endsWith(ws.value), t => s""""$t".endsWith("${ws.value}")""", EndsWith(ws.value))

  implicit def matchesRegexValidate[S <: String](implicit ws: ValueOf[S]): Validate.Plain[String, MatchesRegex[S]] =
  ...
  implicit def startsWithValidate[S <: String](implicit ws: ValueOf[S]): Validate.Plain[String, StartsWith[S]] =
